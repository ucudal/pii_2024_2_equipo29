...
public class FullHeal: IItem
{
    ...
    public string Use(Pokemon currentPokemon)
    {
        ...
        if (currentPokemon.StateMachine.CurrentState.Name == EnumState.Normal.ToString()) return $"El pokemon {currentPokemon.Name} no tiene efectos de ataques especiales."; 
        if (Amount == 0) return "No te quedan super pociones para curar.";
        
        currentPokemon.Hp = currentPokemon.InitialHp;
        Amount -= 1;

        return $"El pokemon {currentPokemon.Name} se ha curado del efecto {currentPokemon.StateMachine.CurrentState.Name}.";
    }
}